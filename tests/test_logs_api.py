...
class TestReceiveLogs:
    ...
    def test_receive_logs_success(self, client):
        ...
        with patch("api.logs.write_to_log_file") as mock_write:
            ...
            )  # At least 2 calls (combined + individual)
    ...
    def test_receive_logs_error_logs(self, client):
        ...
        with patch("api.logs.write_to_log_file") as mock_write:
            ...
            error_calls = [
                call for call in mock_write.call_args_list if call[0][0] == "errors.log"
            ]
...
class TestLogHealthCheck:
    ...
    def test_log_health_check_success(self, client):
        ...
        with patch("api.logs.os.path.exists", return_value=True), patch(
            "api.logs.os.access", return_value=True
        ), patch(
            "api.logs.os.listdir",
            return_value=["frontend.log", "backend.log", "combined.log"],
        ), patch(
            "api.logs.os.path.getsize", return_value=1024
        ):
    ...
    def test_log_health_check_directory_not_exists(self, client):
        ...
        with patch("api.logs.os.path.exists", return_value=False), patch(
            "api.logs.os.access", return_value=False
        ):
            response = client.get("/api/logs/health")
    ...
    def test_log_health_check_with_log_files(self, client):
        ...
        with patch("api.logs.os.path.exists", return_value=True), patch(
            "api.logs.os.access", return_value=True
        ), patch("api.logs.os.listdir", return_value=mock_files), patch(
            "api.logs.os.path.getsize", return_value=2048
        ):
    ...
    def test_log_health_check_file_size_calculation(self, client):
        ...
        with patch("api.logs.os.path.exists", return_value=True), patch(
            "api.logs.os.access", return_value=True
        ), patch("api.logs.os.listdir", return_value=["test.log"]), patch(
            "api.logs.os.path.getsize", return_value=1536
        ):  # 1.5 KB
    ...
    def test_log_health_check_file_not_exists(self, client):
        ...
        with patch("api.logs.os.path.exists", return_value=True), patch(
            "api.logs.os.access", return_value=True
        ), patch("api.logs.os.listdir", return_value=["missing.log"]), patch(
            "api.logs.os.path.getsize", side_effect=FileNotFoundError
        ):
    ...
    def test_log_health_check_error_handling(self, client):
        ...
        with patch(
            "api.logs.os.path.exists", side_effect=Exception("Permission denied")
        ):
            response = client.get("/api/logs/health")
...
class TestWriteToLogFile:
    ...
    def test_write_to_log_file_error_handling(self):
        ...
        with patch("api.logs.open", side_effect=Exception("Permission denied")), patch(
            "api.logs.logger"
        ) as mock_logger:
