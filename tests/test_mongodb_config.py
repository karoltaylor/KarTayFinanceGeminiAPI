...
from src.config.mongodb import MongoDBConfig, get_db, _get_active_env_file, _load_env_once
...
class TestMongoDBConfig:
    ...
    def test_get_mongodb_database_not_set(self):
        ...
        # Test the actual behavior - when MONGODB_DATABASE is not set, it should return the default
        # Since the actual implementation uses os.getenv("MONGODB_DATABASE", "financial_tracker")
        # and the default is "financial_tracker", we can test this directly
        database = MongoDBConfig.get_mongodb_database()
        
        # Should return default value "financial_tracker" (from config file)
    ...
    def test_get_client_success(self, mock_mongo_client):
        ...
        with patch('src.config.mongodb.os.getenv') as mock_getenv:
            ...
            mock_mongo_client.assert_called_once_with("mongodb://localhost:27017", serverSelectionTimeoutMS=10000)
    ...
    def test_get_client_with_connection_string(self, mock_mongo_client):
        ...
        with patch('src.config.mongodb.os.getenv') as mock_getenv:
            mock_getenv.return_value = "mongodb+srv://user:pass@cluster.mongodb.net/test"
            
            client = MongoDBConfig.get_client()
            
            assert client is mock_client
            mock_mongo_client.assert_called_once_with("mongodb+srv://user:pass@cluster.mongodb.net/test", serverSelectionTimeoutMS=10000)
    ...
    def test_get_database_no_database_name(self, mock_get_client):
        ...
        with patch('src.config.mongodb.os.getenv') as mock_getenv:
            ...
            mock_getenv.side_effect = lambda key, default=None: default if key == "MONGODB_DATABASE" else None
    ...
    def test_initialize_collections_success(self, mock_get_database):
        ...
        assert mock_db.users.create_index.call_count >= 3  # email, username, oauth indexes
        ...
        assert mock_db.transactions.create_index.call_count >= 3  # user_id, wallet_id, date indexes
    ...
    def test_get_active_env_file_none_exist(self):
        ...
        with patch('pathlib.Path.exists') as mock_exists:
            ...
            with patch('src.config.mongodb.os.getenv') as mock_getenv:
                ...
                mock_getenv.side_effect = lambda key, default=None: default if key == "ENV_FILE" else None
...
class TestLoadEnvOnce:
    ...
    def test_load_env_once_success(self):
        ...
        with patch('src.config.mongodb._get_active_env_file', return_value="config/config.dev.env"), \
    ...
    def test_load_env_once_no_file(self):
        ...
        with patch('src.config.mongodb._get_active_env_file', return_value=None):
            env_vars = _load_env_once()
