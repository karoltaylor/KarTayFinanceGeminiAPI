...
def sample_csv_file():
    ...
    writer.writerow(
        ["Microsoft", "2024-01-16", "420.25", "5", "2101.25", "2.50", "USD"]
    )
...
class TestTransactionUpload:
    ...
    def test_upload_csv_file_success(
        self, client, test_db, auth_headers, sample_csv_file
    ):
        ...
        assert (
            actual_count >= 3
        ), f"Expected at least 3 transactions, got {actual_count}"
    ...
    def test_upload_to_existing_wallet(
        self, client, test_db, auth_headers, sample_csv_file
    ):
    ...
    def test_upload_different_transaction_types(
        self, client, test_db, auth_headers, sample_csv_file
    ):
        ...
        wallet_id = _create_wallet_and_get_id(
            client, auth_headers, "Wallet_Mixed_Types"
        )
        with open(sample_csv_file, "rb") as f:
            response = client.post(
                "/api/transactions/upload",
    ...
    def test_upload_different_asset_types(
        self, client, test_db, auth_headers, sample_csv_file
    ):
        ...
        wallet_id = _create_wallet_and_get_id(
            client, auth_headers, "Wallet_Auto_Asset_Types"
        )
        with open(sample_csv_file, "rb") as f:
            response = client.post(
                "/api/transactions/upload",
...
class TestTransactionList:
    ...
    def test_list_transactions_after_upload(
        self, client, test_db, auth_headers, sample_csv_file
    ):
    ...
    def test_list_transactions_filter_by_wallet(
        self, client, test_db, auth_headers, sample_csv_file
    ):
        ...
        response = client.get(
            f"/api/transactions?wallet_id={wallet_a_id}", headers=auth_headers
        )
    ...
    def test_list_transactions_user_isolation(
        ...
        wallet2_id = _create_wallet_and_get_id(
            client, auth_headers_user2, "User2 Wallet"
        )
        with open(sample_csv_file, "rb") as f:
            client.post(
                "/api/transactions/upload",
    ...
    def test_list_transactions_invalid_limit(self, client, test_db, auth_headers):
        ...
        response = client.get(
            f"/api/transactions?wallet_id={wallet_id}&limit=0", headers=auth_headers
        )
        assert response.status_code == 422

    def test_list_transactions_limit_exceeds_maximum(
    ...
    def test_list_transactions_negative_skip(self, client, test_db, auth_headers):
        ...
        response = client.get(
            f"/api/transactions?wallet_id={wallet_id}&skip=-1", headers=auth_headers
        )
        assert response.status_code == 422

    def test_list_transactions_filter_nonexistent_wallet(
        self, client, test_db, auth_headers
    ):
        ...
        response = client.get(
            f"/api/transactions?wallet_id={fake_wallet_id}", headers=auth_headers
        )
...
class TestTransactionIntegration:
    ...
    def test_upload_and_verify_transaction_details(
        self, client, test_db, auth_headers, sample_csv_file
    ):
    ...
    def test_multiple_uploads_aggregate_correctly(
        self, client, test_db, auth_headers, sample_csv_file
    ):
    ...
    def test_upload_list_delete_workflow(
        ...
        delete_response = client.delete(
            f"/api/transactions/wallet/{wallet_id}", headers=auth_headers
        )
...
class TestTransactionDelete:
    ...
    def test_delete_wallet_transactions_success(
        self, client, test_db, auth_headers, sample_csv_file
    ):
    ...
    def test_delete_transactions_wallet_not_found(self, client, test_db, auth_headers):
        ...
        response = client.delete(
            f"/api/transactions/wallet/{str(ObjectId())}", headers=auth_headers
        )
    ...
    def test_delete_transactions_wallet_owned_by_another_user(
        self,
        client,
        test_db,
        auth_headers,
        ...
        wallet1_id = _create_wallet_and_get_id(
            client, auth_headers, "User1 Protected Wallet"
        )
        with open(sample_csv_file, "rb") as f:
            client.post(
                "/api/transactions/upload",
        ...
        response = client.delete(
            f"/api/transactions/wallet/{wallet1_id}", headers=auth_headers_user2
        )

        assert response.status_code == 404

        # Verify transactions still exist
        transaction_count = test_db.transactions.count_documents(
            {"wallet_id": wallet1_id}
        )
    ...
    def test_delete_transactions_empty_wallet(self, client, test_db, auth_headers):
        ...
        wallet_id = _create_wallet_and_get_id(
            client, auth_headers, "Empty Wallet", "No transactions"
        )
    ...
    def test_delete_transactions_partial_wallet_isolation(
        self, client, test_db, auth_headers, sample_csv_file
    ):
        ...
        delete_response = client.delete(
            f"/api/transactions/wallet/{wallet_a_id}", headers=auth_headers
        )
        assert delete_response.json()["deleted_count"] == count_a

        # Verify Wallet B transactions remain
        remaining_count = test_db.transactions.count_documents(
            {"wallet_id": wallet_b_id}
        )
        assert remaining_count == count_b

    def test_delete_transactions_special_characters_in_wallet_name(
        self, client, test_db, auth_headers, sample_csv_file
    ):
...
class TestTransactionEdgeCases:
    ...
    def test_upload_very_large_wallet_name(
        self, client, test_db, auth_headers, sample_csv_file
    ):
    ...
    def test_list_transactions_with_wallet_filter_case_sensitivity(
        self, client, test_db, auth_headers, sample_csv_file
    ):
    ...
    def test_concurrent_uploads_to_same_wallet(
        self, client, test_db, auth_headers, sample_csv_file
    ):
    ...
    def test_upload_with_invalid_transaction_type(
        self, client, test_db, auth_headers, sample_csv_file
    ):
        ...
        wallet_id = _create_wallet_and_get_id(
            client, auth_headers, "Test Wallet Invalid Type"
        )
        with open(sample_csv_file, "rb") as f:
            response = client.post(
                "/api/transactions/upload",
    ...
    def test_upload_with_invalid_asset_type(
        self, client, test_db, auth_headers, sample_csv_file
    ):
        ...
        wallet_id = _create_wallet_and_get_id(
            client, auth_headers, "Test Wallet Auto Asset Type"
        )
        with open(sample_csv_file, "rb") as f:
            response = client.post(
                "/api/transactions/upload",
    ...
    def test_list_transactions_max_limit_boundary(
        self, client, test_db, auth_headers, sample_csv_file
    ):
        ...
        response = client.get(
            f"/api/transactions?wallet_id={wallet_id}&limit=1000", headers=auth_headers
        )
