...
def validate_transaction_columns(transaction_data: dict) -> dict:
    ...
    required_fields = [
        'asset_name', 'date', 'volume', 'item_price', 
    ...
    for field, value in transaction_data.items():
        if field == 'volume' and not isinstance(value, (int, float)):
            validation_result['field_types'][field] = f"Expected numeric, got {type(value).__name__}"
            ...
            validation_result['field_types'][field] = f"Expected string, got {type(value).__name__}"
...
def compare_file_to_database_rows(file_info: dict, api_response: dict) -> dict:
    ...
    comparison = {
        ...
        'db_inserted': api_response.get('data', {}).get('summary', {}).get('total_transactions', 0),
        'db_failed': api_response.get('data', {}).get('summary', {}).get('failed_transactions', 0),
        'match_exact': False,
    ...
    success_rate = comparison['db_inserted'] / comparison['file_rows'] if comparison['file_rows'] > 0 else 0
...
class TestAllTestDataFiles:
    ...
    def test_historia_transakcji_csv_original(self, client, test_db, auth_headers):
        ...
        if response.status_code == 200:
            ...
            assert row_comparison['match_reasonable'] or row_comparison['high_success_rate'], (
            ...
            for i, transaction in enumerate(data["data"]["transactions"][:3]):  # Test first 3 transactions
    ...
    def test_file_comparison_historia_transakcji(self, client, test_db, auth_headers):
        ...
        wallet1_id = _create_wallet_and_get_id(client, auth_headers, "Historia Original")
        wallet2_id = _create_wallet_and_get_id(client, auth_headers, "Historia Duplicate")
    ...
    def test_all_files_error_handling(self, client, test_db, auth_headers):
        ...
        for file_path in all_files:
            ...
            wallet_id = _create_wallet_and_get_id(client, auth_headers, f"Error_Test_{file_path.stem}")
    ...
    def test_comprehensive_file_validation(self, client, test_db, auth_headers):
        ...
        for file_path in all_files:
            ...
            # Create wallet for this file
            wallet_id = _create_wallet_and_get_id(client, auth_headers, f"Comprehensive_Test_{file_path.stem}")
            ...
            if response.status_code == 200:
                ...
                # Assertions
                assert row_comparison['db_inserted'] > 0, f"No transactions inserted for {file_path.name}"
                ...
                # For Excel files, expect at least some processing
                    ...
                    assert row_comparison['db_inserted'] >= row_comparison['file_rows'] * 0.05, (
                ...
                assert validation_results_summary['valid_transactions'] > 0, (
        ...
        assert successful_files >= len(all_files) * 0.8, (
